...
public class CustomerDaoJdbc implements CustomerDao {
    ...
    private static final String CREATE_CUSTOMER_SQL =
        "INSERT INTO `customer` (name, address, phone, email, cc_number, cc_exp_date) " + "VALUES (?, ?, ?, ?, ?, ?)";

    private static final String FIND_ALL_SQL =
        "SELECT customer_id, name, address, " + "phone, email, cc_number, cc_exp_date " + "FROM customer";

    private static final String FIND_BY_CUSTOMER_ID_SQL =
        "SELECT customer_id, name, address, " + "phone, email, cc_number, cc_exp_date " + "FROM customer WHERE customer_id = ?";
        ...
        try (PreparedStatement statement = connection
            ...
            if (affected != 1) {
                throw new BookstoreUpdateDbException(
                    "Failed to insert a customer, affected row count = " + affected);
            }
            long customerId;
            ...
            if (rs.next()) {
                ...
                throw new BookstoreUpdateDbException(
                    "Failed to retrieve customerId auto-generated key");
            }
            return customerId;
        }
    ...
    @Override public List<Customer> findAll() {
        ...
        try (Connection connection = getConnection();
            ...
            throw new BookstoreQueryDbException(
                "Encountered problem finding all categories", e);
        }
    ...
    @Override public Customer findByCustomerId(long customerId) {
        ...
        try (Connection connection = getConnection();
            ...
            throw new BookstoreQueryDbException(
                "Encountered problem finding customer " + customerId, e);
        }
        return result;
    }
