 ...
 */
@JsonIgnoreProperties(ignoreUnknown = true) public class ShoppingCart {
     ...
     */
    @JsonIgnore public int getComputedSubtotal() {
        return items.stream().mapToInt(
            item -> item.getQuantity() * item.getBookForm().getPrice()).sum();
    }
