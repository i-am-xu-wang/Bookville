 ...
 */
@Provider @Priority(Priorities.USER) public class ApiExceptionHandler
    ...
    private Response makeResponse(Exception exception, Response.Status status) {
        try {
            ...
            return Response.status(status).entity(writer.getBuffer().toString())
                .type(MediaType.TEXT_PLAIN).build();
        }
        catch (Exception e) {
            logger.log(Level.INFO, e,
                () -> "Problem attempting to map an Exception to a json response");
